  ...
  $.ajax({
    ...
    success: function success(response) {
      ...
      $.each(response, function (i, item) {
        var displayTime = item.timeslot.split('-')[0].trim();
        $atime.append("<option value=\"".concat(displayTime, "\">").concat(displayTime, "</option>"));
      });
...
$('#adate').on('change', function () {
  ...
  $.ajax({
    ...
    success: function success(response) {
      ...
      $atime.empty();
      $.each(response, function (i, item) {
        var fullTime = item.timeslot.trim(); // e.g. "10:00 AM - 11:00 AM"
        var displayTime = fullTime.split('-')[0].trim(); // e.g. "10:00 AM"

        // save remaining slots keyed by full timeslot
        slotsData[fullTime] = item.Remaining;

        // add option
        $atime.append("<option value=\"".concat(fullTime, "\">").concat(displayTime, "</option>"));
      });
...
$('#atime').on('change', function () {
  var selectedTime = $(this).val(); // e.g. "10:00 AM - 11:00 AM"

  if (slotsData[selectedTime] !== undefined) {
    $('#slot-remaining').text("(".concat(slotsData[selectedTime], " slots left)"));
  } else {
    ...
    success: function success(data) {
      ...
      if (data.count.length > 0) {
        $.each(data.count, function (index, detail) {
          list.append("\n                        <li class=\"list-group-item d-flex justify-content-between align-items-center\">\n                            <h4>".concat(detail.atime.split('-')[0].trim(), "</h4>\n                            <span class=\"badge badge-primary badge-pill\">\n                                <h4><strong>").concat(detail.Total, "</strong></h4>\n                            </span>\n                        </li>\n                    "));
        });
      } else {
        list.append("<li class=\"list-group-item\">No appointments found</li>");
      }
      var total = data.total !== null && data.total !== undefined ? data.total : 0;
