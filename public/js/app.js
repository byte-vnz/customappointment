  ...
  $.ajax({
    ...
    success: function success(response) {
      ...
      $.each(response, function (i, item) {
        var fullTime = item.timeslot.trim(); // "10:00 AM - 11:00 AM"
        var displayTime = fullTime.split('-')[0].trim(); // "10:00 AM"
...
$('#adate').on('change', function () {
  ...
  $.ajax({
    ...
    success: function success(response) {
      ...
      $.each(response, function (i, item) {
        var fullTime = item.timeslot.trim();
...
$('#atime').on('change', function () {
  ...
  if (date && slotsData[selectedTime] !== undefined) {
    $('#slot-remaining').text("(".concat(slotsData[selectedTime], " slots left)")).show();
  } else {
    $('#slot-remaining').text('').hide();
  }
});
