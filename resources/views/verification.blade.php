            ...
            <form class="needs-validation" id="master-form" novalidate>
                <div class="card shadow mb-5">
                    <div class="{{ $scheduleIndicator }} card-header text-white text-center">
                        <h5 class=""><i class="fas fa-globe-asia mr-2"></i>{{ config('app.name', 'Appointment') }} Schedule</h5>
                        <h6 class="card-subtitle mt-3">{{ date('M. d, Y', strtotime($appointment->adate)) }} ({{ $appointment->atime }})</h6>
                    </div>
					  ...
					  <div class="card-body">
							...
							<div class="row">
								<div class="col-md-12">
									<h6>FULL NAME:</h6>
									<h5>{{ strtoupper($appointment->lastname) }} {{ strtoupper($appointment->firstname) }}, {{ strtoupper($appointment->middlename) }}</h5>
								</div>
								<div class="col-md-8">
							...
							@if($isExpire)
								<div class="row text-center">
									<div class="col-md-12 col-12 text-danger text-center">YOUR SCHEDULE IS ALREADY CANCELLED OR EXPIRED</div>
								</div>
							@endif
							</div>
